/**
 * Theory: close funded channels first, then ledger balance stays,
 * then create app session with allocations.
 */
...
async function onchainClose(account: any, walletClient: any, publicClient: any, channelId: Hex, closeState: any, serverSig: Hex) {
  ...
  if (chainData[1] === 0) return null; // not on-chain
...
async function main() {
  ...
  // 1. Close Alice's funded channel
  ...
  for (const c of aliceCh.channels?.filter((c: any) => c.status === 'open' && c.amount > 0) || []) {
    const chId = c.channel_id as Hex;
    console.log(`  Closing ${chId.slice(0, 14)}... (amount=${c.amount})`);
    const closeResult = await rpc1.call(
    ...
    const tx = await onchainClose(aliceAccount, aliceWC, publicClient, chId,
      closeResult.state, closeResult.server_signature as Hex);
    if (tx) {
  ...
  for (const c of bobCh.channels?.filter((c: any) => c.status === 'open' && c.amount > 0) || []) {
    const chId = c.channel_id as Hex;
    console.log(`  Closing ${chId.slice(0, 14)}... (amount=${c.amount})`);
    const closeResult = await rpc2.call(
      (s, id) => createCloseChannelMessage(s, chId, BOB, id), bobSigner,
    );
    const tx = await onchainClose(bobAccount, bobWC, publicClient, chId,
      closeResult.state, closeResult.server_signature as Hex);
    if (tx) {
      console.log(`  on-chain close tx: ${tx}`);
      await publicClient.waitForTransactionReceipt({ hash: tx });
  ...
  const openCh = ch.channels?.filter((c: any) => c.status !== 'closed') || [];
  console.log('Alice open channels:', openCh.length);

  // 4. Try app session with allocations
  console.log('\n=== Create App Session (1 USDC each) ===');
  try {
    const result = await rpc3.callCoSigned(
    ...
    if (result.app_session_id) {
      const sessionId = result.app_session_id as Hex;

      // Operate: 1.2 / 0.8
      console.log('\n=== Operate: Alice 1.2, Bob 0.8 ===');
      const opResult = await rpc3.callCoSigned(
      ...
      // Close
      console.log('\n=== Close Session ===');
      const closeResult = await rpc3.callCoSigned(
