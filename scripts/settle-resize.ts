/**
 * Settle the resize on-chain:
 * 1. Get the resize state from ClearNode (already happened, channel is "resizing")
 * 2. On-chain checkpoint → sync state
 * 3. On-chain resize → finalize
 */
...
async function main() {
  ...
  // Take the most recent one (0x3aef...)
  const ch = resizing[0];
  console.log(`\nSettling channel: ${ch.channel_id}`);
  console.log('Full channel data:', JSON.stringify(ch, null, 2));

  // We need the resize state with signatures for on-chain settlement
  // The resize_channel response contained this state, but we didn't save it.
  // Let's try: do another resize_channel to get the state back
  // OR: close the resizing channel and start fresh with a different approach

  // APPROACH: Let's try close_channel on the resizing one, then try a DIFFERENT strategy:
  // Instead of resize after channel creation, try to include the deposit amount
  // in the initial channel creation.

  // Actually, let me first try to get the channel data from on-chain to understand the state
  ...
  // Check on-chain channel data
  console.log('\n=== On-chain channel data ===');
  try {
    const channelData = await nitrolite.getChannelData(ch.channel_id as Hex);
    console.log('Channel data:', JSON.stringify(channelData, (_, v) => typeof v === 'bigint' ? v.toString() : v, 2));
  } catch (e) {
    console.log('getChannelData error:', (e as Error).message);
  }

  // Check on-chain balance
  try {
    const balance = await nitrolite.getAccountBalance('0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48' as Address);
    console.log('Alice on-chain account balance:', balance.toString());
  } catch (e) {
    console.log('getAccountBalance error:', (e as Error).message);
  }

  // Try checkpoint with the resize state
  // The resize response had: {channel_id, state: {intent: 2, version: 1, state_data, allocations: [...], sigs: [...]}}
  // We need to reconstruct this

  // First: do a NEW resize to get the state with signatures
  console.log('\n=== Trying resize again to get signed state ===');
  try {
    const resizeResult = await rpc.call(
      (s, id) => createResizeChannelMessage(s, {
        channel_id: ch.channel_id as Hex,
        resize_amount: 1_000000n,
    ...
    // Now try to do on-chain checkpoint with this state
    if (resizeResult.state) {
      ...
      const candidateState = {
        ...
        data: (state.state_data || '0x') as Hex,
      ...
      try {
        const cpHash = await nitrolite.checkpointChannel({
          channelId: ch.channel_id as Hex,
          candidateState: candidateState as any,
          proofStates: [],
        });
        console.log('Checkpoint tx:', cpHash);
      } catch (e) {
        console.log('Checkpoint error:', (e as Error).message);
      }
    }
  } catch (e) {
    console.log('Resize error:', (e as Error).message);
  }
