...
import { createECDSAMessageSigner, createAuthRequestMessage, createAuthVerifyMessage, createEIP712AuthMessageSigner, createGetConfigMessage, createGetChannelsMessage, createGetLedgerBalancesMessage, createCreateChannelMessage, createResizeChannelMessage, createAppSessionMessage, createGetAppSessionsMessage, NitroliteClient, WalletStateSigner, RPCProtocolVersion, } from '@erc7824/nitrolite';
...
async function main() {
    ...
    const openAliceSessions = aliceSessions?.app_sessions?.filter((s) => s.status === 'open') || [];
    ...
    const aliceOpenCh = aliceCh.channels?.find((c) => c.status === 'open');
    if (!aliceOpenCh) {
        ...
        try {
            ...
            console.log(`createChannel failed, trying depositAndCreateChannel: ${e.message.slice(0, 200)}`);
        ...
        for (let i = 1; i <= 24; i++) {
            ...
            const found = ch.channels?.find((c) => c.channel_id === chResult.channel_id && c.status === 'open');
            if (found) {
        ...
        const resizeResult = await rpc.call((s, id) => createResizeChannelMessage(s, {
            channel_id: chResult.channel_id,
        ...
        const { request } = await publicClient.simulateContract({
            ...
            args: [channelId, { ...unsignedResize, sigs: [userSig, resizeResult.server_signature] }, [initProof]],
    ...
    const openCh = aliceCh2.channels?.find((c) => c.status === 'open' && BigInt(c.amount) > 0n);
    if (openCh) {
        console.log(`\nTrying allocate_amount=-${DEPOSIT} on channel ${openCh.channel_id.slice(0, 20)}...`);
        try {
            const result = await rpc.call((s, id) => createResizeChannelMessage(s, {
                channel_id: openCh.channel_id,
            ...
            console.log(`allocate_amount=-DEPOSIT error: ${e.message}`);
        }
        ...
        try {
            ...
            console.log(`resize=-DEPOSIT allocate=0 error: ${e.message}`);
        }
    }
    ...
    try {
        ...
        try {
            ...
            console.log('Asymmetric session error:', e2.message);
        }
        ...
        try {
            ...
            console.log('Zero-alloc session error:', e3.message);
        }
    }
    rpc.close();
    console.log('\nDone!');
...
//# sourceMappingURL=test-allocate.js.map