/**
 * Minimal test: direct deposit to Custody → check ledger → create session
 */
...
async function main() {
  ...
  // 3. Wait for ClearNode to detect
  ...
  for (let i = 1; i <= 30; i++) {
    ...
    const amt = bal.ledger_balances?.find((b: any) => b.asset === 'usdc')?.amount || '0';
    if (i % 3 === 0 || parseFloat(amt) > 0) console.log(`  ${i * 5}s: usdc=${amt}`);
    if (parseFloat(amt) > 0) { ledgerUpdated = true; break; }
  }
