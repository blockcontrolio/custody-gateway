...
import { createECDSAMessageSigner, createAuthRequestMessage, createAuthVerifyMessage, createEIP712AuthMessageSigner, createGetChannelsMessage, createGetLedgerBalancesMessage, createCloseChannelMessage, createAppSessionMessage, createCloseAppSessionMessage, createGetAppSessionsMessage, RPCProtocolVersion, } from '@erc7824/nitrolite';
...
async function main() {
    ...
    for (const c of aliceCh.channels?.filter((c) => c.status !== 'closed') || []) {
        const chId = c.channel_id;
        console.log(`  Closing ${chId.slice(0, 20)}... (status=${c.status}, amount=${c.amount})`);
        try {
            const closeResult = await rpc.call((s, id) => createCloseChannelMessage(s, chId, ALICE, id), aliceSigner);
    ...
    for (const sess of sessions?.app_sessions?.filter((s) => s.status === 'open') || []) {
        ...
        try {
            ...
            console.log(`  Session close error: ${e.message.slice(0, 200)}`);
        }
    }
    ...
    const nonClosed = aliceCh2.channels?.filter((c) => c.status !== 'closed') || [];
    ...
    for (let i = 1; i <= 30; i++) {
        ...
        const usdcBal = bal.ledger_balances?.find((b) => b.asset === 'usdc');
        const amount = usdcBal?.amount || '0';
        if (i % 3 === 0 || amount !== '0')
            console.log(`  ${i * 5}s: ledger usdc=${amount}`);
        if (parseFloat(amount) > 0) {
            console.log('Ledger updated!');
            break;
        }
        if (i === 30)
            console.log('No ledger update after 150s');
    }
    ...
    const finalNonClosed = finalCh.channels?.filter((c) => c.status !== 'closed') || [];
...
//# sourceMappingURL=test-direct-deposit.js.map