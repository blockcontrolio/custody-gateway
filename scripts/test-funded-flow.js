...
async function getPrivateKey(address) {
    ...
    return decrypt(row.encryptedKey, row.iv, row.tag);
}
class ClearNodeRPC {
    ws;
    pending = new Map();
    ...
    async call(buildMessage, signer) {
        const requestId = this.nextId++;
        const msg = await buildMessage(signer, requestId);
        return this.sendRaw(requestId, msg);
    }
    async callCoSigned(buildMessage, primarySigner, allSigners) {
        const requestId = this.nextId++;
...
async function main() {
    ...
    console.log('\n=== Auth Alice2 ===');
    const { createAuthRequestMessage, createAuthVerifyMessage, createEIP712AuthMessageSigner, } = await import('@erc7824/nitrolite');
    ...
    const walletLikeClient = {
        ...
        signTypedData: (args) => aliceAccount.signTypedData(args),
    };
    ...
    const balances = await rpc.call((s, id) => createGetLedgerBalancesMessage(s, undefined, id), aliceSigner);
    console.log('Balances:', JSON.stringify(balances, null, 2));
    const USDC_MAINNET = '0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48';
    const ethNetwork = config.networks.find((n) => n.chain_id === 1);
    if (!ethNetwork) {
        console.error('No Ethereum mainnet in config');
        process.exit(1);
    }
    const CUSTODY_ADDRESS = ethNetwork.custody_address;
    const ADJUDICATOR_ADDRESS = ethNetwork.adjudicator_address;
    const BROKER = config.broker_address;
    console.log('Custody:', CUSTODY_ADDRESS, 'Broker:', BROKER);
    const DEPOSIT_AMOUNT = 1000000n;
    ...
    async function ensureFundedChannel(name, address, key, account, signer) {
        console.log(`\n=== Ensure Funded Channel (${name}: ${address}) ===`);
        const ch = await rpc.call((s, id) => createGetChannelsMessage(s, address, undefined, id), signer);
        let openChannel = ch.channels?.find((c) => c.status === 'open' && c.token === USDC_MAINNET);
        ...
        if (!openChannel) {
            ...
            const channelResult = await rpc.call((s, id) => createCreateChannelMessage(s, { chain_id: 1, token: USDC_MAINNET }, id), signer);
            ...
            for (let i = 1; i <= 24; i++) {
                ...
                const updated = await rpc.call((s, id) => createGetChannelsMessage(s, address, undefined, id), signer);
                openChannel = updated.channels?.find((c) => c.channel_id === channelResult.channel_id && c.status === 'open');
                if (openChannel) {
                    console.log(`✅ Detected (${i * 5}s)`);
                    break;
                }
                if (i % 6 === 0)
            ...
            if (!openChannel)
                throw new Error(`Channel ${channelResult.channel_id} not detected after 2 min`);
        }
    ...
    const bobWalletLike = { account: bobAccount, signTypedData: (args) => bobAccount.signTypedData(args) };
    ...
    const balancesAfter = await rpc.call((s, id) => createGetLedgerBalancesMessage(s, undefined, id), aliceSigner);
    console.log('Balances:', JSON.stringify(balancesAfter, null, 2));
    console.log('\n=== Create App Session (1 USDC each) ===');
    try {
        const participants = [ALICE2, BOB];
        const appSessionResult = await rpc.callCoSigned((s, id) => createAppSessionMessage(s, {
            definition: {
...
//# sourceMappingURL=test-funded-flow.js.map